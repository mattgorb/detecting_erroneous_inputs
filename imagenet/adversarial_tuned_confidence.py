...
l2=True
if l2:
    att = CarliniL2Method(classifier, confidence=conf, targeted=False, learning_rate=0.01, binary_search_steps=10,max_iter=10, initial_const=0.01, max_halving=5, max_doubling=5, batch_size=1)
else:
    att = carlini.CarliniLInfMethod(classifier, confidence=conf, targeted=False, learning_rate=0.01,max_iter=10, max_halving=5, max_doubling=5, eps=0.3, batch_size=1)

attack_data  = generate_attacks(model, att)
np.save('imagenet/combined/carlini_l2_'+str(conf)+'conf_'+str(l2)+'_' + str(name) + '_' + str(type) + '.npy', np.array(attack_data))
