...
def train(train_data,length, balance='balanced', fprN=False, file=None):
    #balance={0:5, 1:1}
...
if save:
    ...
    incorrect = np.concatenate([incorrect, cifar100, corrupted, sun, fgsm, c_linf, c_l2, ])
    ...
    names=[0]
    for i in names:
        incorrect=loadNPY('cifar10/all_random/'+str(i)+'.npy')
