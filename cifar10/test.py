...
if train_all:
    ...
    import pickle
    cifar100 = pickle.load(open('cifar10/generated_test/unseen_w_classes_.dump', 'rb'))
    cifar100=np.array(cifar100)
    print(cifar100.shape)
    train_data = np.concatenate(
        [[correct[i] for i in range(len(cifar100))], [cifar100[i][1] for i in range(len(cifar100))]])
    train(train_data, len(cifar100), fprN=fpr)
