...
class Attacker(ch.nn.Module):
    ...
    def forward(self, x, target, device,*_, constraint, eps, step_size, iterations,
...
class AttackerModel(ch.nn.Module):
    ...
    """
    def __init__(self, model, dataset, device, one_output=False):
    ...
    def forward(self, inp, target=None, make_adv=False, with_latent=False,
        ...
        if make_adv:
            ...
            self.eval()
            adv = self.attacker(inp, target,self.device, **attacker_kwargs)
            if prev_training:
                self.train()
