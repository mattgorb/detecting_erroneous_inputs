...
from . import imagenet_models, cifar_models, tiny_imagenet_models
...
class TinyImageNet(DataSet):
    ...
    * Insect (classes 300-319)
    ...
    def __init__(self, data_path, **kwargs):
        """
        """
        ds_name = 'tiny_imagenet_models'
        ds_kwargs = {
            'num_classes': 200,
            'mean': ch.tensor([0.4717, 0.4499, 0.3837]),
            'std': ch.tensor([0.2600, 0.2516, 0.2575]),
            'custom_class': None,
            'label_mapping': get_label_mapping(ds_name,200),
            'transform_train': da.TRAIN_TRANSFORMS_IMAGENET,
            'transform_test': da.TEST_TRANSFORMS_IMAGENET
        }
        super(TinyImageNet, self).__init__(ds_name,
                data_path, **ds_kwargs)
    ...
    def get_model(self, arch, pretrained):
        ...
        return tiny_imagenet_models.__dict__[arch](num_classes=self.num_classes)
...
DATASETS = {
    ...
    'cinic': CINIC,
    'a2b': A2B,
    'tiny_imagenet_models': TinyImageNet
}
'''
