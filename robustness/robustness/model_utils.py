...
def make_and_restore_model(*_, arch, dataset, device, resume_path=None,parallel=True, pytorch_pretrained=False,one_output=False):
    """
    ...
    model = AttackerModel(classifier_model, dataset,device,one_output)
    ...
    if resume_path:
        if os.path.isfile(resume_path):
            ...
            if device=='cpu':
                checkpoint = ch.load(resume_path, map_location=device,pickle_module=dill)
            else:
